...
def summary_10fold_results(summary_dir):
    ...
    print("Mean Accuracy = {0}".format(df_group.loc[best_epoch]['val_accuracy']))
    print("Mean std = {0}".format(df_group.loc[best_epoch]['std']))

    # Document the validation results of the best epoch, per experiment
    df2 = df[df.epoch == best_epoch].copy()#[:10]
